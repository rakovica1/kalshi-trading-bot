...
@click.option("--qualified-only", is_flag=True, help="Only show qualified markets (Tier 1 + top 100 $vol + $1k+ + <5% spread).")
...
def scan_cmd(ctx, min_price, min_volume, prefixes, show_sizing, qualified_only, sort_by, reverse_sort):
    ...
    try:
        ...
        click.echo(f"Qualified (Tier 1 + Top 100 + $1k+ + <5% spread): {len(qualified)}")
...
def whale_trade(ctx, prefixes, min_price, min_volume, max_positions,
    ...
    Qualified = Tier 1 (>=98c) + top 100 by $vol + >=$1k daily + <5% spread.
